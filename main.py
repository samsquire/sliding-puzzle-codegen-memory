...
def update_node(my_id, openSet, neighbour, gScore, myCameFrom, fScore, current, queue, queues, end):
  ...
  if tentative_gScore < gScore[neighbour]:
      ...
      if neighbour not in openSet:
          ...
          for index, queue in enumerate(queues):
            if my_id != index: 
              queue.put((my_id, "update", (neighbour, gScore[neighbour], fScore[neighbour], True)))
      else:
        for index, queue in enumerate(queues):
          if my_id != index: 
            queue.put((my_id, "update", (neighbour, gScore[neighbour], fScore[neighbour], False)))
...
def A_Star(start, goal, h, fScore, worker_len):
      ...
      while len(openSet) > 0:
        ...
        for index, neighbour in enumerate(find_neighbours(current, end)):
          ...
          if worker_len > index % worker_len == my_id:
            ...
            update_node(my_id, openSet, neighbour, gScore, myCameFrom, fScore, current, my_queue, queues, end)
  ...
  for i in range(0, worker_len):
    ...
    worker = multiprocessing.Process(target=a_worker, args=(i, queues[i], parent, queues))
